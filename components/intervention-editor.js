(function(){
  ...
  polymer_ext({
    ...
    make_javascript_editor: async function(editor_div){
      var intervention_name, self, brace, aceRange, js_editor, intervention_info, style, markedLines;
      ...
      if (intervention_name != null) {
        ...
        style = $('<style>.error_highlight{position:absolute;z-index:20;border-bottom: 1px dotted red;}</style>');
        $('body').append(style);
        markedLines = [];
        js_editor.getSession().on('change', async function(e){
          var current_time, prev_text, current_text, errors, i$, ref$, len$, marker, annotations, error, endLine, endColumn;
          ...
          for (i$ = 0, len$ = errors.length; i$ < len$; ++i$) {
            ...
            marker = js_editor.getSession().addMarker(new aceRange(error.line - 1, error.column - 1, endLine - 1, endColumn - 1), "error_highlight", "line");
